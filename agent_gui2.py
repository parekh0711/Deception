...
def print_result(lynched, players):
        ...
        for condition in win_condition:
            ...
            if condition[0] == "grudge":
                ...
                if lynched == condition[2]:
                    ...
                    for player in players:
                        if player.name == condition[1]:
                            losers.append([player.name, player.role])
                    remove_player(condition[1], players)
    ...
    for player in players:
        if player.name in win_list:
            send_info(player.number, "Well played. "+trophy)
            print(player.name, player.role)
        else:
            send_info(player.number, "You are a " +clown+".")
            losers.append([player.name, player.role])
    if losers == []:
        return
    print("\nLosers are:")
    ...
    def __init__(self, window):
        def callback(event):
            ...
            if 239 <= event.x <= 302 and 404 <= event.y <= 435:
                ...
                rows = cur.fetchall()
                nms = [row[0] for row in rows]
                print("--------------------------------------------------------")
                print("Votes are:")
                # [print(nme) for nme in nms]
                c= Counter(nms)
                [print(nme,c[nme]) for nme in c.keys()]
                print("--------------------------------------------------------")
                lynched = max(set(nms), key = nms.count)
                cur.close()
                conn.commit()
                conn.close()
                global players
